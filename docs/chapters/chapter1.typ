...
Traditionally, detection and assessment of stuttering rely on manual observation by speech-language pathologists, which can be subjective, time- consuming, and often inconsistent across experts.
...
The project aims to develop an intelligent system that can automatically detect and classify various types of stuttering in speech, including repetitions, prolongations, blocks, and fillers, using deep learning techniques.
The input audio will be first preprocessed and converted into embeddings, which will be then analyzed using a CNN and Transformer-based architecture to learn both local acoustic patterns and long-range temporal dependencies that differentiate normal and dysfluent speech.
Along with stutter classification, the same audio will be also passed through a transcription module, and the detected stutter timestamps will be hard-aligned with the transcript timestamps to identify the exact word, syllable, or speech segment where the dysfluency occurs.
The final model will be integrated into a user-friendly interface that allows users to record or upload speech samples and receive real-time analysis, timestamped detection results, and classification output.
...
A deep learning–based approach using Convolutional Neural Networks (CNNs) and audio spectrogram analysis can automatically learn 
