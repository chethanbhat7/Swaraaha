...
The final output is delivered through an interactive visualization interface and a detailed clinical style report.
...
These requirements are structured as a set of functional units (FR1–FR11), each corresponding to a specific capability within the system.

*FR1 - Speech Recording:*
...
The system shall generate a detailed analysis report in a clinical format and maintain user history using local storage mechanisms such as LocalStorage or IndexedDB.

*FR11 - Model Management:*
...
*Usability:*
The interface is designed to be intuitive and accessible for both clinicians and non-technical users. Features such as a clean graphical user interface, dark mode, and guided workflows help users navigate the system with ease.
...
The project is supported by the availability of publicly accessible datasets such as Boli, SEP-28K, and UCLASS. These datasets can be obtained through platforms like GitHub and Kaggle. An automated pipeline is used to download, merge, and preprocess the data, ensuring efficient dataset preparation within the project timeline.
