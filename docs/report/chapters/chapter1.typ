...
Speech fluency lets people express thoughts and emotions clearly, but many experience interruptions in their speech flow due to a disorder known as stuttering.
Stuttering is a common speech disorder characterized by involuntary repetitions, prolongations, or blocks during speech production.
It affects individuals across all age groups and can impact their confidence, communication ability, and social interactions.
Identifying stuttering patterns early helps with effective therapy and management.
...
The project aims to develop a system that automatically detects, classifies, and analyzes types of stuttering in speech, including repetitions, prolongations, blocks, and fillers, using deep learning techniques.
...
The final model will be integrated into a simple interface that allows users to record or upload speech samples and receive analysis, timestamped detection results, and classification output.
...
It also supports the wider use of artificial intelligence in healthcare for diagnosis, therapy, and quality of life.
...
These limitations point to the need for a reliable, accessible stutter detection system.
A deep learning–based approach using Wav2Vec2 embeddings and audio spectrogram analysis can learn complex acoustic patterns and temporal dependencies that distinguish normal speech from dysfluent speech, allowing accurate classification and localization of various stuttering types.
...
- To preprocess and standardize speech audio by reducing noise, normalizing volume, and preparing it for accurate stutter detection.
- To develop an automated stutter detection system that accurately identifies and classifies five types of stuttering: Prolongation, Block, Sound Repetition, Word Repetition, and Interjection.
- To implement a temporal localization module capable of identifying the exact timestamps of stuttering events, the affected region, and the duration within a speech sample.
...
- Creating a cross-platform GUI using PyQt5 that enables users to record, upload, analyze, and visualize speech data interactively.
...
- Providing an accessible tool that supports automation of speech pathology assessments and assists in early identification, monitoring, and therapy tracking for individuals with stuttering.
