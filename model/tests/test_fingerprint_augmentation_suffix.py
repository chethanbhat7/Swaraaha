...
def test_single_class_augmentation_on_no_suffix():
    from model.fingerprint import fingerprint
    ...
    assert not fingerprint(args).endswith("_naug")


def test_single_class_missing_augmentation_attr_no_suffix():
    from model.fingerprint import fingerprint
...
def test_multitask_augmentation_on_no_suffix():
    from model.fingerprint import multitask_fingerprint
    ...
    args = Namespace(
        ...
        batch_size=16, max_length_seconds=3.0, warmup_steps=500, weight_decay=0.01,
        freeze_backbone_epochs=3, loss_type="focal", focal_gamma=2.0, seed=42,
        gradient_accumulation_steps=1, epochs=20, augmentation=True,
    )
    assert not multitask_fingerprint(args).endswith("_naug")


def test_cnn_augmentation_on_no_suffix():
    from model.fingerprint import cnn_classifier_fingerprint
    ...
    assert not cnn_classifier_fingerprint(args).endswith("_naug")


def test_localizer_augmentation_on_no_suffix():
    from model.fingerprint import localizer_fingerprint

    cnn = Namespace(
        data_dir="data/train", epochs=30, batch_size=8, lr=1e-3, n_mels=128,
        hop_length=512, max_length_seconds=3.0, dropout=0.4, patience=7,
        weight_decay=1e-4, seed=42, val_ratio=0.2, augmentation=True,
    )
    assert not localizer_fingerprint(cnn, "loc").endswith("_naug")
    ...
    w2v2 = Namespace(
        ...
        weight_decay=0.01, freeze_backbone_epochs=5,
        model_name="facebook/wav2vec2-base", seed=42, val_ratio=0.2,
        warmup_steps=500, augmentation=True,
    )
    assert not localizer_fingerprint(w2v2, "wav2vec").endswith("_naug")


def test_localizer_legacy_parse_implies_augmentation_on():
    cnn_fp = "cnnloc_e30_b8_lr0.001_n128_h512_ml10_d0.4_pa7_wd0.0001_vr0.2_s42_train"
    assert parse_localizer_fingerprint(cnn_fp)["augmentation"] is True

    w2v2_fp = ("w2v2loc_e20_b4_lr3e-5_frz5_wu500_hd256_d0.3_wd0.01_"
               "ml10_pa5_vr0.2_s42_train_w2v2base")
    assert parse_localizer_fingerprint(w2v2_fp)["augmentation"] is True
