...
$MULTI --data_dir "$DATA_DIR" --output_dir "$OUT_DIR" \
  --model_name facebook/wav2vec2-base --freeze_backbone_epochs 3 --no-augmentation

echo "=== arm03: multitask w2v2 frz20 (no-aug) ==="
$MULTI --data_dir "$DATA_DIR" --output_dir "$OUT_DIR" \
  --model_name facebook/wav2vec2-base --freeze_backbone_epochs 20 --no-augmentation
...
$MULTI --data_dir "$DATA_DIR" --output_dir "$OUT_DIR" \
  --model_name microsoft/wavlm-base-plus --no-augmentation
$MULTI --data_dir "$DATA_DIR" --output_dir "$OUT_DIR" \
  --model_name facebook/hubert-base-ls960 --no-augmentation
